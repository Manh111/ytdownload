// Production server entry point for cPanel deployment
...
app.post('/api/youtube', async (req, res) => {
  try {
    ...
    if (!apiKey) {
      console.error("[Production] RAPIDAPI_KEY not configured");
      return res.status(500).json({ 
        error: "API key not configured. Please set RAPIDAPI_KEY in environment variables." 
      });
    }

    console.log("[Production] Fetching video info for ID:", videoId);
    ...
    for (const endpoint of endpoints) {
      try {
        console.log("[Production] Trying endpoint:", endpoint);
        ...
        if (response.ok) {
          ...
          console.log("[Production] Success with endpoint:", endpoint);
          break;
        } else {
          console.log("[Production] Failed with endpoint:", endpoint, "Status:", response.status);
        }
      } catch (err) {
        console.log("[Production] Error with endpoint:", endpoint, err.message);
        continue;
      }
    }
    ...
    if (!response || !response.ok) {
      ...
      console.error("[Production] All endpoints failed. Last error:", errorText);
    ...
    console.log("[Production] Successful endpoint:", apiEndpoint);

    const data = await response.json();
    console.log("[Production] API Response data:", data);
    ...
    console.error("[Production] API Route Error:", error);
...
app.post('/api/youtube/playlist', async (req, res) => {
  try {
    ...
    console.log("[Production] Fetching playlist info for ID:", playlistId);
    ...
    if (!response.ok) {
      ...
      console.error("[Production] Playlist API error:", response.status, errorText);
    ...
    const data = await response.json();
    console.log("[Production] Playlist API Response:", data);
    
    res.json(data);
  } catch (error) {
    console.error("[Production] Playlist API Error:", error);
    res.status(500).json({ error: "Internal server error", details: error.message });
  }
});
...
app.post('/api/youtube/related', async (req, res) => {
  try {
    ...
    console.log("[Production] Fetching related videos for ID:", videoId);
    ...
    if (!response.ok) {
      const errorText = await response.text();
      console.error("[Production] Related videos API error:", response.status, errorText);
    ...
    const data = await response.json();
    console.log("[Production] Related videos API Response:", data);
    
    res.json(data);
  } catch (error) {
    console.error("[Production] Related videos API Error:", error);
    res.status(500).json({ error: "Internal server error", details: error.message });
  }
});
...
app.get('/api/proxy-download', async (req, res) => {
  try {
    ...
    console.log(`[Production] Proxy fetching: ${url}`);
    ...
    if (!response.ok) {
      console.error(`[Production] Proxy HTTP error: ${response.status} ${response.statusText}`);
      return res.status(response.status).json(
    ...
    console.log(`[Production] Proxy success: ${contentType}, ${contentLength ? `${contentLength} bytes` : 'unknown size'}`);
...
app.listen(PORT, () => {
  console.log(`[Production] YouTube Downloader server running on port ${PORT}`);
